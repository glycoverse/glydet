...
`glydet` provides five trait factory functions for creating custom derived traits:
...
- `wmean()` for calculating the weighted mean of a quantitative property, weighted by glycan abundance
- `total()` for calculating the total abundance of a specific glycan subset
- `wsum()` for calculating the weighted sum of a quantitative property, weighted by glycan abundance
...
For instance, `Tp` represents glycan type, `nM` denotes the number of mannose residues, and so forth.
...
|------|----------|------|-------------|
| `Tp` | `n_glycan_type()` | factor | Type of the glycan, either "complex", "hybrid", "highmannose", or "pausimannose" |
...
When `trait_fns` is omitted, `derive_traits()` internally invokes `traits_basic()` to utilize built-in trait functions.
