...
- [`quantify_motifs()`](https://glycoverse.github.io/glydet/dev/reference/quantify_motifs.md)
  now supports `dynamic_motifs()` and `branch_motifs()` from glymotif.
...
- [`quantify_motifs()`](https://glycoverse.github.io/glydet/dev/reference/quantify_motifs.md)
  now includes a `motif_structure` column in the `var_info` tibble of
  the returned experiment, containing the parsed glycan structure for
  each motif.
...
- The first parameter of
  ...
  is renamed from `val_cond` to `val`.
